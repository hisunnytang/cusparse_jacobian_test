...
void test_batchQR()
{
    // test matrix multiplication with batchQR solve
    // create an overall system vector

    int batchsize = 1;
    int nchem     = 5;

    int N = batchsize * nchem;
  ...
  /* create right-hand size vector for linear solver */
...
//                                    block_nnz);  /* number of nonzeros in a subsystem */
    ...
    * and column indicies are not zeroed out by SUNMatZero */
    ...
    // and read the jacobian to matrix A
    abc_data data = abc_setup_data(NULL, NULL);
    abc_read_cooling_tables( &data);
    abc_read_rate_tables( &data);
    realtype *Jdata, *ydata;
    ydata = N_VGetDeviceArrayPointer_Cuda(y);
    Adata = SUNMatrix_cuSparse_Data(A);
    int block_size = 32;
    int grid_size = (N + block_size - 1) / block_size;
    jacobian_kernel<<<grid_size, block_size>>>(ydata, Adata, *data);
    cudaDeviceSynchronize();
    cudaError_t cuerr = cudaGetLastError();
    if (cuerr != cudaSuccess) {
        fprintf(stderr, ">>> ERROR in Jac: cudaGetLastError returned %s\n",
